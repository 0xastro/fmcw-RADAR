...
typedef struct MCB_t
{
    ...
    DSS_DataPathObj             dataPathObj[NUM_SUBFRAMES];           ///!  Data Path object
...
#ifdef DSS
extern void _dssAssert(int32_t expression, const char *file, int32_t line);
#define dssAssert(expression) {                                      \
                                    _dssAssert(expression,           \
                                    __FILE__, __LINE__);             \
